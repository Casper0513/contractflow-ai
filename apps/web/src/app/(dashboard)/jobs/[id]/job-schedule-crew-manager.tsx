...
type JobScheduleCrewManagerProps = {
  jobId: string;
  schedule: JobSchedule;
};

export function JobScheduleCrewManager({ jobId, schedule }: JobScheduleCrewManagerProps) {
  ...
  async function assignCrewMember() {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/jobs/${jobId}/schedules/${schedule.id}/crew`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        setError(responseData?.message ?? "Unable to assign this crew member.");
  ...
  async function removeCrewMember(crewMemberId: string) {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/jobs/${jobId}/schedules/${schedule.id}/crew/${crewMemberId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        setError(responseData?.message ?? "Unable to remove this crew member.");
